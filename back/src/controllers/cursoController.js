...
async function obtenerCursosTotales(req, res) {
    ...
    const resultado = await pool.request()
      ...
      .query(`
        SELECT
          ...
          CuposDisponibles AS cuposDisponibles,
          Activo AS activo
        FROM Cursos
        WHERE Id = @id
      `);
...
async function crearCurso(req, res) {
  ...
  try {
    ...
    if (!nombre || !categoria || !duracion || !cuposDisponibles || activo === undefined) {
    ...
    const resultado = await pool.request()
      ...
      .query(`
        ...
        OUTPUT INSERTED.Id AS id, INSERTED.Nombre AS nombre, INSERTED.Categoria AS categoria, INSERTED.Duracion AS duracion, INSERTED.CuposDisponibles AS cuposDisponibles, INSERTED.Activo AS activo
